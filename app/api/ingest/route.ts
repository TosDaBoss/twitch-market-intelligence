...
import { getFullIngestionData, getTopGlobalStreamsWithFollowers } from "@/lib/twitch";
import { supabaseAdmin } from "@/lib/supabaseAdmin";

// FULL ingestion — runs every 4 hours
// Fetches top games + creators WITH follower counts + global top 100, updates daily stats
...
export async function GET(request: NextRequest) {
  ...
  try {
    const [snapshots, globalCreators] = await Promise.all([
      getFullIngestionData(5, 25),
      getTopGlobalStreamsWithFollowers(100),
    ]);
    ...
    for (const snapshot of snapshots) {
      ...
      for (const creator of creators) {
        await upsertCreatorWithDailyStats(creator, game.id, now, today);
      }
    }

    // Store global top 100 creators with follower counts + daily stats
    for (const creator of globalCreators) {
      // Upsert their game if present
      if (creator.gameId) {
        await supabaseAdmin.from("games").upsert({
          id: creator.gameId,
      ...
      await upsertCreatorWithDailyStats(
        creator,
        creator.gameId || null,
        now,
        today
...
async function upsertCreatorWithDailyStats(creator: any, gameId: string | null, now: string, today: string) {
  // Upsert creator
  ...
  if (existingCreatorStat) {
    await supabaseAdmin
      ...
      .update({
        ...
        follower_count: creator.followerCount || existingCreatorStat.follower_count,
        follower_growth: creator.followerCount
          ? creator.followerCount - existingCreatorStat.follower_count
          : existingCreatorStat.follower_growth,
      })
